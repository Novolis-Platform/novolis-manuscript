...
public static class BookPrintAssembler
{
        ...
        ManuscriptChapterMetadata meta)
    {
        ...
        var i = 0;
        while (i < lines.Length && (string.IsNullOrWhiteSpace(lines[i]) || IsHtmlCommentLine(lines[i])))
            i++;
        ...
        if (i < lines.Length && IsAtxHeading(lines[i], out var level, out var headingText) && level == 1)
        {
            ...
            // Strip legacy [!tag] / plain public mirrors under H1 (stardate-led or callout-led).
            if (i < lines.Length && ManuscriptMetadata.IsPublicDatelineStartLine(lines[i]))
            {
                while (i < lines.Length && ManuscriptMetadata.IsDatelineQuoteLine(lines[i]))
                    i++;
                while (i < lines.Length && string.IsNullOrWhiteSpace(lines[i]))
                    i++;
            }
        ...
        // Leading HTML comments before H1 are author-only (dropped from reader body).
        var body = i >= lines.Length ? "" : string.Join('\n', lines.Skip(i));
        return (headingMarkdown, title, body);
    }

    static bool IsHtmlCommentLine(string line)
    {
        var t = line.Trim();
